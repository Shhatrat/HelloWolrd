...
public class AnimationViewOld extends View {
    ...
    private CircleRefreshLayout.OnDrawListener onDrawListener;

    public void setOnDrawListener(CircleRefreshLayout.OnDrawListener onDrawListener) {
    ...
    public AnimationViewOld(Context context) {
        this(context, null, 0);
    }

    public AnimationViewOld(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public AnimationViewOld(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void drawListener(Canvas canvas, CircleRefreshLayout.DrawRefreshType type){
        ...
        onDrawListener.changeType(type);
        layout.draw(canvas);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        switch (mAniStatus) {
            case PULL_DOWN:
                ...
                drawListener(canvas, CircleRefreshLayout.DrawRefreshType.DISABLE);
                break;
            case REL_DRAG:
                drawDrag(canvas);
                drawText(canvas, "down");
            ...
            case DRAG_DOWN:
                drawDrag(canvas);
                drawListener(canvas, CircleRefreshLayout.DrawRefreshType.REFRESHING);
                break;
            case SPRING_UP:
            ...
            case DONE:
                drawDone(canvas);
                drawListener(canvas, CircleRefreshLayout.DrawRefreshType.DONE);
                invalidate();
                break;
            case STOP:
                drawDone(canvas);
    ...
    private float getRelRatio() {
        if (System.currentTimeMillis() >= mStop || force) {
            springUp();
            return 1;
        }
